...
void query_data( size_t pos, bool compact )
{
  ...
  if ( compact && ( /*(pos % (all_names.size()/10) == 0) ||*/ (pos >= all_names.size())) )
  {
    ...
    WLOG_BEGIN("Сжатие базы на " << size_t( (pos * 100)  / all_names.size() ) << "%");
    to.compact([pos](){
...
void process_data( size_t pos, btpclient::aggregated_list&& agl )
{
  ...
  WLOG_PROGRESS( std::setw(7) << (pos * 100.0)  / all_names.size() << "% " << logname << ": "  << agl.size() << "      " )
  /*if ( !agl.empty()  )
  {*/
...
int main(int argc, char* argv[])
{
  ...
  if (argc > 5 )
    max_lag = std::atol(argv[5]);
  if (argc > 6 )
    min_points =std::atol(argv[6]); 
  ...
  from.start(io_service, argv[1], argv[2]);
  to.start(io_service, argv[3], argv[4]);
