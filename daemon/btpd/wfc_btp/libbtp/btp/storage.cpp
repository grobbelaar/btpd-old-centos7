...
bool storage::open(const storage_options& opt, std::string* err)
{
  ...
  if ( !_key_storage->open(opt.key_db, err) )
    return false;
  /*!!
  BTP_LOG(trace, "Compact key storage...");
  ...
  */
  ...
  /*!!!
  BTP_LOG(trace, "Compact data storage...");
...
bool storage::get(const std::string& name, aggregated_list* result, std::string* err, time_type ts, size_t offset, size_t limit)
{
  ...
  {
    time_t death = (time(0) - _ttl) * _resolution;
...
/*
std::vector<std::pair< std::string, time_type> > storage::get_names( 
    const std::string& prefix, 
    const std::string& suffix,
    size_t offset,
    size_t limit,
...
{
  return _key_cache->get_names(prefix, suffix, offset, limit, by, power);
}
*/
