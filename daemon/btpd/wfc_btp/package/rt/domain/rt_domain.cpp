...
{
  for (std::string name: this->options().targets)
    _targets.push_back( this->get_target<istatistics>( name ) );
}

void rt_domain::ready() 
{
  ...
  {
    _multi_add_stat = stat->create_time_factory("multi_add:rate");
    _add_stat = stat->create_time_factory("add:rate");
    _add_batch_cnt = stat->create_value_factory("add:batch");
    _values_stat = stat->create_value_factory("values:batch");
  }
}


void rt_domain::ag_handler_(const std::string& name, aggregated_ptr ag)
{
...
void rt_domain::push( wfc::statistics::request::push::ptr req, wfc::statistics::response::push::handler cb) 
{
  ...
  bool status = _aggregators->push(name, *req, std::bind(&rt_domain::ag_handler_, this, std::cref(name), _1 ));
...
void rt_domain::add( rtdepr::request::add::ptr req, rtdepr::response::add::handler cb) 
{
  ...
  bool status = _aggregators->push(req->name, ag, std::bind(&rt_domain::ag_handler_, this, std::ref(name), _1 ));
...
void rt_domain::multi_add( rtdepr::request::multi_add::ptr req, rtdepr::response::multi_add::handler cb) 
{
  ...
  wfc::time_meter  multi_add_mtr;
  wfc::time_meter  add_stat_mtr;
  wfc::value_meter add_batch_mtr;
  wfc::value_meter values_mtr;
  ...
  for ( auto& a : req->data )
  {
    ...
    _aggregators->push(name, ag, std::bind(&rt_domain::ag_handler_, this, std::cref(name), _1 ));
  }
